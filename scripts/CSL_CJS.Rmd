...
# Priors and constraints
  ...
  #males
    ...
    for (t in 3:(n_occasions-1)) {
      logit(phi2M[i,t]) <- mu.2.3M #+ eps2[t] 
      logit(p3[i,t]) <- mu.p2.3 #+ eps.pJ[t]
    }
    
    for (t in 4:(n_occasions-1)) { 
      logit(phi3M[i,t]) <- mu.2.3M #+ epsJ[t] 
      logit(p4[i,t]) <- mu.p4.6 #+ eps.pJ[t]
    } #t psi
    
    for (t in 5:(n_occasions-1)) {
      logit(phi4M[i,t]) <- mu.4.6M #+ epsJ[t] 
      logit(p5[i,t]) <- mu.p4.6 #+ eps.pA[t]
    } #t 
    
    for (t in 6:(n_occasions-1)) {  
      logit(phi5M[i,t]) <- mu.4.6M #+ epsA[t] 
      logit(p6[i,t]) <- mu.p4.6 #+ eps.pA[t]
    } #t 
    
    for (t in 7:(n_occasions-1)) {  
      logit(phi6M[i,t]) <- mu.4.6M #+ epsA[t] 
      logit(p7M[i,t]) <- mu.p7.12M #+ eps.pA[t]
    } #t 
    
    for (t in 8:(n_occasions-1)) {  
      logit(phi7M[i,t]) <- mu.7.9M #+ epsA[t] 
      logit(p8M[i,t]) <- mu.p7.12M #+ eps.pA[t]
    } #t 
    
    for (t in 9:(n_occasions-1)) {  
      logit(phi8M[i,t]) <- mu.7.9M #+ epsA[t] 
      logit(p9M[i,t]) <- mu.p7.12M #+ eps.pA[t]
    } #t 
    
    for (t in 10:(n_occasions-1)) {  
      logit(phi9M[i,t]) <- mu.7.9M #+ epsA[t] 
      logit(p10M[i,t]) <- mu.p7.12M #+ eps.pA[t]
    } #t 
    
    for (t in 11:(n_occasions-1)) {  
      logit(phi10M[i,t]) <- mu.10.12M # + epsA[t] 
      logit(p11M[i,t]) <- mu.p7.12M #+ eps.pA[t]
    } #t 
  
    for (t in 12:(n_occasions-1)) {  
      logit(phi11M[i,t]) <- mu.10.12M # + epsA[t] 
      logit(p12M[i,t]) <- mu.p7.12M #+ eps.pA[t]
    } #t   
    
        for (t in 13:(n_occasions-1)) {  
      logit(phi12M[i,t]) <- mu.10.12M # + epsA[t] 
      logit(pAM[i,t]) <- mu.pAM #+ eps.pA[t]
    } #t  
    
        for (t in 14:(n_occasions-1)) {  
      logit(phi13M[i,t]) <- mu.13.16M # + epsA[t] 
    } #t  
    
        for (t in 15:(n_occasions-1)) {  
      logit(phi14M[i,t]) <- mu.13.16M # + epsA[t] 
    } #t  
    
        for (t in 16:(n_occasions-1)) {  
      logit(phi15M[i,t]) <- mu.13.16M # + epsA[t] 
    } #t  
    
        for (t in 17:(n_occasions-1)) {  
      logit(phi16M[i,t]) <- mu.13.16M # + epsA[t] 
    } #t  
    
        for (t in 18:(n_occasions-1)) {  
      logit(phiAM[i,t]) <- mu.AM # + epsA[t] 
    } #t  
  } #males
...
params <- c('int.p1', 'int.p2.3', 'int.p4.6', 'int.p7.12', 'int.pA', 'int.p7.12M', 'int.pAM',
            ...
            'epsP', 'eps1', 'eps2', 'epsJ', 'epsA',
            'phiP.prob', 'phi1.prob', 'phi2.prob', 'phi3.prob',  'phi4.prob', 'phi5.prob',
            'phi6.prob',  'phi7.prob',  'phi8.prob','phi9.prob', 
            'phi10.prob',  'phi11.prob',  'phi12.prob','phi13.prob',  
            'phi14.prob',  'phi15.prob',  'phi16.prob',  
            'phiA.prob',
            'int.phiP', 'int.phi1', 'int.phi2.3', 'int.phi4.6', 'int.phi7.9',
            'int.phi10.12', 'int.phi13.16', 'int.phiA',
            'int.phiPM', 'int.phi1M', 'int.phi2.3M', 'int.phi4.6M', 'int.phi7.9M',
